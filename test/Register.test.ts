...
describe("Register", async () => {
  ...
  it("Should return a lot of values", async () => {
    for (let i of categories) await expect(register.updateCategory(i[0], i[1], i[2], i[3])).to.emit(register, "CategoryUpdatedEvent")
    ...
    for (let i = 0; i < markets.length; i++) {
      ...
      ).to.emit(register, "PredictionMarketRegisteredEvent")
    }
  ...
  it("Should update old data", async () => {
    await expect(register.updateCategory(categories[0][0], categories[0][1], categories[0][2], categories[0][3])).to.emit(
      register,
      "CategoryUpdatedEvent"
    )
    await expect(
      register.updateMilestone(milestones[0][0], milestones[0][1], milestones[0][2], milestones[0][3], milestones[0][4], 0)
    ).to.emit(register, "MilestoneUpdatedEvent")
    ...
    await expect(
      register.registerMarket(
        addr,
        markets[0][0],
        parseInt(markets[0][1]),
        markets[0][2],
        markets[0][3],
        AddressZero,
        parseInt(markets[0][4]),
        parseInt(markets[0][4]),
        outcomes[0]
      )
    ).to.emit(register, "PredictionMarketRegisteredEvent")
    ...
    await expect(register.updateCategory(categories[0][0], "123", categories[0][2], categories[0][3])).to.emit(
      register,
      "CategoryUpdatedEvent"
    )
    await expect(register.updateMilestone(milestones[0][0], milestones[0][1], "123", milestones[0][3], milestones[0][4], 0)).to.emit(
      register,
    ...
    expect(data[0][0].position).to.be.equal(123)
    expect(data[1][0].position).to.be.equal(123)
    expect(data[2][0].position).to.be.equal(123)

    let outcome = outcomes[0][0]
    outcome.position = 123
    await register.addOutcome(addr, outcome.uuid, outcome.position, outcome.name)

    data = await register.getPredictionData(await now(ethers.provider))

    expect(data[3].length).to.be.equal(2)
    expect(data[3][0].position).to.be.equal(123)
  })
})
