...
describe("MarsERC20Token", async () => {
  ...
  it("Only owner can transfer tokens", async () => {
    ...
    await expect(marsToken.connect(users[0])["mint(address,uint256,uint256)"](await users[0].getAddress(), 1000, 0)).to.be.revertedWith(
      errorMessage
    )

    await expect(marsToken.connect(owner)["mint(address,uint256,uint256)"](await users[0].getAddress(), 1000, 0)).to.be.not.revertedWith(
      errorMessage
    )
  })
  ...
  it("EmissionController can transfer as much as he wants", async () => {
    ...
    await expect(marsToken.connect(eC)["mint(address,uint256)"](await users[0].getAddress(), 1000)).to.be.revertedWith(errorMessage)

    await marsToken.connect(owner).setEmissionController(eCa)

    await expect(
      marsToken.connect(eC)["mint(address,uint256)"](await users[0].getAddress(), tokens(1_000_000_000_000))
  ...
  it("Coreteam once received can't transfer tokens until specified date", async () => {
    ...
    await expect(marsToken.connect(users[0]).transfer(await users[1].getAddress(), 1000)).to.be.revertedWith(
      "MarsERC20: Tokens are locked until lockPeriod"
    )
  })
  ...
  it("Other participants, once received can transfer tokens at any time", async () => {
    ...
    await expect(marsToken.connect(users[0]).transfer(await users[1].getAddress(), 1000)).to.be.not.revertedWith(
      "MarsERC20: Tokens are locked until lockPeriod"
    )
  })
  ...
  it("Only owner can change lockPeriod", async () => {
    ...
    await expect(marsToken.connect(users[0]).setLockPeriod(12345)).to.be.revertedWith(errorMessage)

    expect((await marsToken.getLockPeriod()).toNumber()).to.be.equal(1683774000)

    await expect(marsToken.connect(owner).setLockPeriod(12345)).to.be.not.revertedWith(errorMessage)
