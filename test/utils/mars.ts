...
import { MarsAddressResolver, MarsPredictionMarketFactory, TestERC20, MarsPredictionMarket__factory, MarsPredictionMarket } from "../../typechain"
...
export const deployMars = async (ethers: Ethers, owner: Signer): Promise<Mars> => {
  ...
  const addressResolver = (await (await ethers.getContractFactory("MarsAddressResolver"))
    .connect(owner)
    .deploy()) as MarsAddressResolver

  const marsToken = (await (await ethers.getContractFactory("TestERC20")).connect(owner).deploy(tokens(1000000), "Test Token", 18, "TST")) as TestERC20
  ...
  const predictionMarketFactory = (await (await ethers.getContractFactory("MarsPredictionMarketFactory"))
    ...
    let tx = await predictionMarketFactory.connect(owner).createMarket(
      MILESTONE, 1, "Example", "Example", marsToken.address, timeEnd, 
